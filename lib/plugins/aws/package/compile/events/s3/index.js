...
class AwsCompileS3Events {
    ...
    this.serverless.service.getAllFunctions().forEach(functionName => {
      ...
      if (functionObj.events) {
        functionObj.events.forEach(event => {
          ...
          if (event.s3 && _.isObject(event.s3) && event.s3.existing) return null;
  ...
  existingS3Buckets() {
      ...
      if (functionObj.events) {
        functionObj.events.forEach((event, idx) => {
          if (event.s3 && _.isObject(event.s3) && event.s3.existing) {
            ...
            rules = _.map(event.s3.rules, (value, key) => ({
            ...
            const customS3Resource = {
              [customS3ResourceLogicalId]: {
                ...
                Properties: {
                  ...
                  FunctionName: service.getFunction(functionName).name,
                  BucketName: bucket,
                  BucketConfig: {
                    Event: notificationEvent,
                    Rules: rules,
    ...
    if (iamRoleStatements.length) {
      // TODO: this was copied over from `mergeIamTemplates.js`
      // DRY it out
      const policyDocumentStatements = this.serverless.service.provider
      ...
      return addCustomResourceToService.call(this, 's3');
    }
